import { createRouter, createWebHistory } from "vue-router";
import { useAuthStore } from "../store/AuthStore";
// import addDrone from "../views/addDrone.vue";
// import Home from "../views/Home.vue";
// import Login from "../views/Login.vue";
// import Register from "../views/Register.vue";

const routes = [
  {
    path: "/",
    component: () => import("../views/Login.vue"),
    name: "Login",
  },
  {
    path: "/home",
    name: "home",
    component: () => import("../views/Home.vue"),
    meta: {
      requiresAuth: true,
    },
  },

  {
    path: "/register",
    component: () => import("../views/Register.vue"),
    name: "Register",
  },
  {
    path: "/add-drone",
    component: () => import("../views/inventory/addDrone.vue"),
    name: "addDrone",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/add-payload",
    component: () => import("../views/inventory/addPayload.vue"),
    name: "addPayload",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/add-battery",
    component: () => import("../views/inventory/addBattery.vue"),
    name: "addBattery",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/add-drone-model",
    component: () => import("../views/model/addDroneModel.vue"),
    name: "addDroneModel",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/add-payload-model",
    component: () => import("../views/model/addPayloadModel.vue"),
    name: "addPayloadModel",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/add-battery-model",
    component: () => import("../views/model/addBatteryModel.vue"),
    name: "addBatteryModel",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/show-drone",
    component: () => import("../views/inventory/show/showDrone.vue"),
    name: "showDrone",
    meta: {
      requiresAuth: true,
    },
  },
  {
    path: "/list-drones",
    component: () => import("../views/inventory/list/listDrones.vue"),
    name: "listDrones",
    meta: {
      requiresAuth: true,
    },
  },
];

const router = createRouter({
  history: createWebHistory(),
  routes,
});

router.beforeEach((to) => {
  const store = useAuthStore();

  if (to.meta.requiresAuth && !store.isAuth) return "/";
});

export default router;
